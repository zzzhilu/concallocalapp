...
async def api_save_meeting(req: SaveMeetingRequest):
    """Save a meeting record."""
    title = req.title or datetime.now().strftime("會議 %Y-%m-%d %H:%M")
    result = save_meeting(
        title=title,
...
async def api_list_meetings():
    ...
    return JSONResponse(list_meetings())


@app.get("/api/meetings/{meeting_id}")
async def api_get_meeting(meeting_id: str):
    """Get a full meeting record."""
